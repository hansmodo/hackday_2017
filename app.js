...
//const RE_PEOPLE       = ['Abigail Adams','Abigail Smith','John Adams','John Quincy Adams'].join('|');
...
function sendMsgPluginLoaded(activeTabId){
  ...
  if(activeTabId){
    ...
    chrome.tabs.sendMessage(activeTabId, { 'plugin':'load','stylesheet':chrome.extension.getURL('/css/content.css'),'regex':RE_PEOPLE }, function(response) {
      ...
      if(response && response.content_data){
        ...
        //let uri = Model.get('url');
        //let logPayload = Object.assign(Util.getBaseLogPayload(getUserId), {edmo_content_url:uri});
        //console.log("...logPayload:",logPayload);
        //mixpanel.track('browser_extension_desktop:content:fetched', logPayload);
      }else {
        console.log("no_active_tab_response");
        //when user is logged in but has just installed w/existing tabs open...
        //if(authStatus){
          //document.querySelector(Util.makeQuerySelector(Conf.CSS.PRMPT_RELOAD)).classList.add(Conf.CSS.ACTIVE);
        //}
        //mixpanel.track('browser_extension_desktop:no_active_tab_response:error', Util.getBaseLogPayload(getUserId));
      }
    });
  }
};
